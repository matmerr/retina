...
type KapingerMeshClient struct {
	client        http.Client
...
func NewKapingerMeshClient(clientset *kubernetes.Clientset, labelselector string, volume int, interval time.Duration, httpPort int) (*KapingerMeshClient, error) {
	k := KapingerMeshClient{
		client: http.Client{
...
func (k *KapingerMeshClient) MakeRequests(ctx context.Context) error {
	ticker := time.NewTicker(k.interval)
	for {
		select {
		case <-ctx.Done():
			slog.Info("Mesh client context done")
			return nil
		case <-ticker.C:
			go func() {
...
func (k *KapingerMeshClient) makeRequest(ctx context.Context, url string) ([]byte, error) {
...
func (k *KapingerMeshClient) getServiceURLs() ([]string, error) {
	urls := []string{}
...
func (k *KapingerMeshClient) getPodURLs() ([]string, error) {
	urls := []string{}
